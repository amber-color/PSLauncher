...
function Show-AppEditDialog {
    ...
    function New-Label { param([string]$Text, [int]$Y)
    ...
    function New-TextBox { param([int]$Y, [int]$Width = 330)
    ...
    function New-BrowseButton { param([int]$Y, [int]$X = 430)
        $b = New-Object System.Windows.Forms.Button
        $b.Text      = '...'
        $b.Location  = New-Object System.Drawing.Point($X, $Y)
        $b.Size      = New-Object System.Drawing.Size(44, 24)
    ...
    # 表示名
    $lblName   = New-Label '表示名' 16
    $txtName   = New-TextBox 14 330
    $txtName.Text = if ($isNew) { '' } else { $App.name }

    # 実行パス
    $lblPath   = New-Label '実行パス' 52
    $txtPath   = New-TextBox 50 280
    $txtPath.Text = if ($isNew) { '' } else { $App.path }

    $btnBrowsePath = New-BrowseButton 50 380
    ...
    # アイコンパス
    $lblIcon   = New-Label 'アイコン' 88
    $txtIcon   = New-TextBox 86 280
    $txtIcon.Text = if ($isNew) { '' } else { $App.iconPath }

    $btnBrowseIcon = New-BrowseButton 86 380
    ...
    # グループ
    $lblGroup  = New-Label 'グループ' 124
    $txtGroup  = New-TextBox 122 330
    $txtGroup.Text = if ($isNew) { '' } else { $App.group }

    # 説明ラベル（アイコン省略可能）
    $lblHint = New-Object System.Windows.Forms.Label
    $lblHint.Text     = '※ アイコンは省略可。実行ファイルから自動取得します。'
    $lblHint.Location = New-Object System.Drawing.Point(12, 158)
    $lblHint.Size     = New-Object System.Drawing.Size(460, 18)
    ...
    $btnOk.Location  = New-Object System.Drawing.Point(290, 232)
    ...
    $btnCancel.Location = New-Object System.Drawing.Point(383, 232)
